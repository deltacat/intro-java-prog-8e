...
public class SpinnerModelEditorDemo extends JApplet {
	...
	private JSpinner jspMonth = new JSpinner(new SpinnerListModel(Arrays
			.asList(monthNames).subList(0, 12)));
	private JSpinner spinnerYear = new JSpinner(new SpinnerNumberModel(2004, 1,
			3000, 1));
	...
	public SpinnerModelEditorDemo() {
		...
		// Set editor for date
		JSpinner.DateEditor dateEditor = new JSpinner.DateEditor(jspDate,
				"MMM dd, yyyy");
	...
	private void updateDate() {
		...
		int month = ((SpinnerListModel) jspMonth.getModel()).getList().indexOf(
				jspMonth.getValue());
